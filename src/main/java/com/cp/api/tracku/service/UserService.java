...
public class UserService {
	...
	public ResponseEntity<CommonResponse> processResetPasswordAndStatusRequest(UserPasswordReset upr, boolean isFirstLogin) {
		CommonResponse cr = new CommonResponse();
		...
		if (status) {
			...
			if(isFirstLogin) {
				System.out.println("is here login");
				repository.updateUserStatus(upr.getId(), upr.getClient());
			}
		} else {
			cr.setCode("1002");
